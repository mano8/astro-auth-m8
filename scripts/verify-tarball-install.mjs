...
const EXPECTED_PACKAGE_FILES = [
  ...
  "dist/src/runtime/authorization.js",
  "dist/src/runtime/authorization.d.ts",
  "dist/src/runtime/errors.js",
  "dist/src/runtime/react/index.js",
  "src/routes/login.astro",
  "src/routes/logout.astro",
  "src/routes/callback.astro",
  "src/routes/account.astro",
  "src/scaffold/styles/auth.css"
];
...
function verifyExports(installedRoot) {
  ...
  for (const subpath of EXPECTED_EXPORT_SUBPATHS) {
    ...
    const specifier = subpath === "." ? "@mano8/astro-auth-m8" : `@mano8/astro-auth-m8/${subpath.slice(2)}`;
  ...
  assert(
    packageJson.faAuthM8?.contract === "fa-auth-m8@2.0",
    `unexpected published contract: ${packageJson.faAuthM8?.contract}`
  );
}
...
function main() {
  ...
  const installedRoot = join(WORK_DIR, "node_modules", "@mano8", "astro-auth-m8");
